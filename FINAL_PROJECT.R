#censusData<-read.csv("C:\\Users\\KhaulA\\R_Practise\\FINAL_PROJECT\\census-income_.csv", stringsAsFactors=T)
#str(censusData)
#table(is.na)
...
ggplot(censusData,aes(x=relationship,fill=race))+
  ...
  labs(x="Categories of Relationships",y="Count of Categories", fill="")
...
ggplot(censusData,aes(x=age, fill=X))+geom_histogram(bins =90)+
  labs(title = "Distribution of Age")+theme_dark()
